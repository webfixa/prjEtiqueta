...
public class TelaOrcamento extends JFrame {
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jLbTipoEtiqueta.setBounds(new Rectangle(26, 481, 109, 16));
	...
	private JTextField getJTfFilial() {
		if (jTfFilial == null) {
			...
			jTfFilial.setBounds(new Rectangle(142, 20, 76, 20));
	...
	private JButton getJBtIncluir() {
		if (jBtIncluir == null) {
			...
			jBtIncluir.setBounds(new Rectangle(25, 130, 105, 36));
					...
					for(Iterator<Orcamento> it=ctrl.getOrcamento(jTfFilial.getText(), jTextField.getText()).iterator();it.hasNext();)
						{
							...
							linha[0]= orc.getCODITPROD();
							linha[1]=orc.getDESCRICAO();  
					        linha[2]=orc.getCODBARRA();  
					        try{
					        linha[3]= nf.format(Double.parseDouble(orc.getPRECOUNIT())); }
					        catch(NumberFormatException x){
					        	String valor="";
					        	char[] prec = new char[orc.getPRECOUNIT().length()];
					        	for(int i=0;i<orc.getPRECOUNIT().length();i++)
					        	{
					        		prec[i]= orc.getPRECOUNIT().charAt(i);
					        	}
					        	boolean ponto = false;		
					        ...
					        try{
					        	linha[4]= String.valueOf(Integer.parseInt(orc.getQTCOMP()));}
						        catch(NumberFormatException x){
						        	String valor="";
						        	char[] prec = new char[orc.getQTCOMP().length()];
						        	for(int i=0;i<orc.getQTCOMP().length();i++)
						        	{
						        		prec[i]= orc.getQTCOMP().charAt(i);
						        	}
						        	boolean ponto = false;		
						        	for(int i=0;i<prec.length;i++)
						        	{
	...
	private JButton getJBtExcluir() {
		if (jBtExcluir == null) {
			...
			jBtExcluir.setBounds(new Rectangle(131, 130, 108, 36));
	...
	private JButton getJBtLimpar() {
		if (jBtLimpar == null) {
			...
			jBtLimpar.setBounds(new Rectangle(240, 130, 123, 36));
	...
	private JButton getJBtLimparLista() {
		if (jBtLimparLista == null) {
			...
			jBtLimparLista.setBounds(new Rectangle(364, 130, 105, 36));
	...
	private JScrollPane getJScrollPane() {
		if (jScrollPane == null) {
			...
			jScrollPane.setBounds(new Rectangle(26, 195, 732, 274));
	 ...
	 * @return javax.swing.JTable	
	 *
	//mï¿½todo para atualizar (preencher) a tebela  
	 public void atualizaTabelaCliente()  
	    {  
	        ...
	        insereTabelaCliente(clientes);//pasando por parï¿½metro o ArrayList de clientes  
	    }  
	...
	private void insereTabelaCliente(ArrayList<Cliente> clientes)  
	    {  
	      ...
	      //Aqui eu defino as colunas que a tabela vai conter     
	       dtmc.addColumn("Cï¿½digo");  
	       dtmc.addColumn("Nome");  
	       dtmc.addColumn("CPF");  
	       dtmc.addColumn("Data de Nascimento");  
	       dtmc.addColumn("Endereï¿½o");  
	       ...
	       dtmc.addColumn("Pendï¿½ncias");  
	       dtmc.addColumn("Crï¿½ditos");  
	       dtmc.addColumn("Dependentes");  
	       dtmc.addColumn("Nï¿½mero de locaï¿½ï¿½es");  
	       ...
	       //Aqui eu preencho a tabela com os dados de todos os clientes do Array Clientes  
	        for(Object c2 : clientes){  
	           ...
	           // 17 ï¿½ o nï¿½mero de colunas da tabela, se vocï¿½ colocar um numero menor vai dar erro  
	              
	            linha[0]=cliente.getCodigoS();//cï¿½digo do cliente na coluna 0  
	        ...
	        //somente para aquelas que serï¿½o um pouco mais largas como o campo "NOME" e "ENDEREï¿½O"  
	...
	Para pega um dado na linha selecionada pelo usuï¿½rio:
	private void botaoClicado() { //coloque o codigo a baixo dentro da anction de um botao   
	//Pegar dados da tabela  
	 if(tabelaClientes.getSelectedRowCount() == 1){ //verifico se somente uma linha estï¿½ selecionada  
	       String assa = (String) tabelaClientes.getValueAt(tabelaUser.getSelectedRow(), 0);  
	       //Pego o valoe da coluna 0 na linha que foi selecionada pelo usuï¿½rio;  
	...
	private JButton getJBtImprimir() {
		if (jBtImprimir == null) {
			...
			jBtImprimir.setLocation(new Point(392, 477));
	...
	private JCheckBox getJChBPreco() {
		if (jChBPreco == null) {
			...
			jChBPreco.setBounds(new Rectangle(566, 478, 21, 21));
		}
		return jChBPreco;
	}
	...
	private JTextField getJTfFilial1() {
		if (jTfFilial1 == null) {
			...
			jTfFilial1.setBounds(new Rectangle(218, 20, 322, 20));
	...
	private JTextField getJTextField() {
		if (jTextField == null) {
			...
			jTextField.setBounds(new Rectangle(142, 46, 104, 20));
